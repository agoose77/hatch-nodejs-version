    ...
    def template(self) -> str:
        if self.__template is None:
            template = self.config.get(
                "template",
                """# File generated by hatch_nodejs_version
# Don't change it
...
""",
            ...
            if not isinstance(template, str):
                raise TypeError(
                    f"Option `template` for version source `{self.PLUGIN_NAME}` must be a string"
                )
    ...
    def version_file(self) -> str:
        if self.__version_file is None:
            version_file = self.config.get("version-file", "")
            if not isinstance(version_file, (str, bytes, os.PathLike)):
                raise TypeError(
                    f"Option `version-file` for version source `{self.PLUGIN_NAME}` must be a string"
                )
            if version_file and not (os.path.splitext(version_file)[-1] == ".py"):
                raise ValueError(
                    f"Option `version-file` for version source `{self.PLUGIN_NAME}` must be a Python file; got `{version_file}`"
                )
    ...
    def set_version(self, version: str, version_data):
        ...
        # Ignore if version_file is an empty string
        if self.version_file:
            (Path(self.root) / self.version_file).write_text(
                self.template.format(version=version)
            )
